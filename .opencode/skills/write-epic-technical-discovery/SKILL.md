...
1. Save the epic to `initiatives/[initiative-name]/output/epic-discovery-[slug].md`.
...
5. Then ask: *"Want me to create this epic in Jira?"* — if yes, invoke the `jira` skill and **pass the saved file path** so the `jira` skill can write back the issue key and URL into the file's frontmatter after creation.
